...
export default function Login(){
  const navigate=useNavigate();const[email,setEmail]=useState('');const[password,setPassword]=useState('');const[errors,setErrors]=useState({});const[loading,setLoading]=useState(false);const[message,setMessage]=useState('');
  ...
  async function submit(e){e.preventDefault();setMessage('');if(!validate())return;setLoading(true);try{await authApi.login({email:email.trim().toLowerCase(),password});navigate('/dashboard',{replace:true})}catch(err){setMessage(err.data?.message||'Invalid email or password.')}finally{setLoading(false)}}
